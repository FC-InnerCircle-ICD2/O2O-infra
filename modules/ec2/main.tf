...
# Database EC2가 사용할 private address를 생성합니다.
resource "aws_network_interface" "database_app" {
  subnet_id       = var.private_subnet_ids[2]
  private_ips     = [ "10.0.5.100" ]
  security_groups = [ var.vpc_security_group.id ]

  tags = {
    Name = "prod-database-nic-5"
  }
}
...
resource "aws_instance" "database_app_instance" {
  ...
  network_interface {
    network_interface_id  = aws_network_interface.database_app.id
    device_index          = 0
  }
